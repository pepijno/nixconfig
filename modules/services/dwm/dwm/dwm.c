...
enum { SchemeNorm, SchemeSel, SchemeTag, SchemeTag1, SchemeTag2, SchemeTag3, SchemeTag4, SchemeTag5 }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - stw - x) > bh_n) {
		drw_setscheme(drw, scheme[SchemeNorm]);
		drw_rect(drw, x, 0, w - 2 * sp, bh_n, 1, 1);
	}
...
runautoclose(void)
{
	...
	/* if $XDG_DATA_HOME is set and not empty, use $XDG_DATA_HOME/dwm,
	 * otherwise use ~/.local/share/dwm as autoclose script directory
	 */
	xdgdatahome = getenv("XDG_DATA_HOME");
	if (xdgdatahome != NULL && *xdgdatahome != '\0') {
		/* space for path segments, separators and nul */
